        ...
        setTimeout(()=>{
            try {
                ...
                if (postArray.length===0){
                    postArray = Array.from(document.querySelectorAll('._4-u2.mbm._4mrt._5jmm._5pat._5v3q._7cqq._4-u8'));
                    postArray.forEach((elem) => {
                        const title = elem.querySelector('._7tae._14f3._14f5._5pbw._5vra').innerText;
                        const origDate = elem.querySelector('.timestampContent').innerText;
                        const date = getDate(origDate);
                        let text = elem.querySelector('._5pbx.userContent._3576');
                        if (text !== null) {
                            text = text.innerText;
                        }
                        let image = elem.querySelector('.scaledImageFitWidth.img');
                        if (image !== null) {
                    ...
                    postArray.forEach(elem => {
                        ...
                        stringArray.push(parent.innerHTML);
                    });
                    postArray.forEach((elem) => {
                ...
                chrome.runtime.sendMessage({"posts": resArray, "close": true, "defaultArray": stringArray, 'debug': true, "pageCode": page.innerHTML});
            }
            catch (e) {
